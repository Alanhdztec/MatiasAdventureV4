...
func takeDamage(danio, source_position: Vector2):
	if not can_be_hit:
		return
		
	vida -= danio
	print("¡Jugador golpeado! Vida: ", vida)

	# Calcular knockback
	var direction = (global_position - source_position).normalized()
	velocity = direction * knockback_force
	move_and_slide()  # Si usas CharacterBody2D, probablemente ya estés usando esto en _physics_process
	
	# Temporizador de invulnerabilidad para evitar múltiples golpes seguidos
	can_be_hit = false
